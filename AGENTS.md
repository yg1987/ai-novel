...
`understand`、`understand-chat` 及相关 `understand-anything` 能力由用户主动调用。除非用户明确要求，否则不得自行触发或将其作为本项目任务的前置步骤。代码相关任务仍必须遵守上述 `codegraph` 红线规则。

## 执行纪律

### 可选探针必须隔离

可能正常返回“无匹配”或路径可能不存在的命令（如 `rg`、可选目录读取、第三方声明探测）不得与必需读取放在同一个并行批次。必须先确认路径存在，或单独执行并独立处理非零退出码，避免一个可选探针使整批结果失效。
