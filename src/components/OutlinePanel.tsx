...
export default function OutlinePanel({ projectId, segmentSize, onSegmentSizeChange, onNavigateToWriting }: Props) {
  ...
  return <div className="panel-layout">
    ...
    {startTarget && <Modal className="chapter-create-modal"><h2>从细纲开始写作</h2><p>{startTarget.volume} / 第{chapterOrder(startTarget.chapterId)}章</p>{startTarget.chapterId === 'ch001' && <label>卷名（可选）<input autoFocus value={volumeName} onChange={(event) => setVolumeName(event.target.value)} placeholder="留空显示默认卷名" /></label>}<label>章节名（可选）<input autoFocus={startTarget.chapterId !== 'ch001'} value={chapterName} onChange={(event) => setChapterName(event.target.value)} placeholder="留空显示默认章节名" onKeyDown={(event) => { if (event.key === 'Enter') void startWriting() }} /></label>{actionError && <p className="error-bar">{actionError}</p>}<div className="material-modal-footer"><button onClick={() => setStartTarget(null)}>取消</button><button disabled={starting} onClick={() => { void startWriting() }}>{starting ? '创建中…' : '创建正文并开始写作'}</button></div></Modal>}
  </div>
}
